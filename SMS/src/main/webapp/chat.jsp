...
<%@page import="sms.db.ChatStudent"%>
<%@page import="sms.model.ChatStudentDao"%>
...
.sideDiv {
	width: 100%;
	min-height: 60px;
	background: linear-gradient(to right, rgba(208, 240, 162, 1) 2%,
		rgba(69, 117, 1, 1) 47%);
	color: red;
}
...
<body>
	...
	<form action="studentWelcome.jsp">
		<input type="submit" value="back"
			style="float: right; margin-right: 30px; margin-top: 20px; background-color: gray; color: white;">
	</form>
	<div class="mainDiv">
		<div class="">
			<div id="">
			...
			<div id="friends">
				<table>
					...
					<tr>
						...
						<td
							style="font-size: 25px; padding-top: 10px; text-decoration: none; color: black; padding-left: 20px;"><a
							href="sms.controller.ChatPage?name=<%=tr.getName()%>&emailid=<%=tr.getEmailid()%>"
							style="text-decoration: none; color: black;"><%=tr.getName()%></a></td>
						<%
							if (tr.getStatus() != null) {
						%>
						<td><img src="images/online.png"
							style="float: right; height: 20px; width: 20px; position: absolute; right: 20px;"></td>
						<%
							} else {
						%>
						<td><img src="images/ofline.png"
							style="float: right; height: 20px; width: 20px; position: absolute; right: 20px;"></td>
						<%
							}
						%>
